...
def basic_file_copy_example():
    ...
    print(f"Throughput: {result.transfer_rate / (1024 * 1024):.2f} MB/s")
...
def progress_monitoring_example():
    ...
    def progress_callback(progress):
        ...
        print(
            ...
            f"Speed: {progress.transfer_rate / (1024 * 1024):.2f} MB/s"
        )
    ...
    result = asyncio.run(
        ferrocp.copy_file(
            str(large_file), str(dest_file), progress_callback=progress_callback
        )
    )
...
def directory_copy_example():
    ...
    # Copy entire directory. `copy_directory` always walks the tree, so there
    # is no `recursive` flag on CopyOptions to set.
    options = ferrocp.CopyOptions()

    print(f"Copying directory {src_dir} to {dest_subdir}")

    result = asyncio.run(
        ferrocp.copy_directory(str(src_dir), str(dest_subdir), options)
    )
