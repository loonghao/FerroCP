    ...
    c.bench_function("engine_selection_performance", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            for (name, size) in files {
                ...
                black_box(selector.select_optimal_engine(&source, &dest).await.unwrap());
            }
        });
    });
}
    ...
    c.bench_function("threshold_adjustment_performance", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            for _ in 0..100 {
                ...
                selector.update_performance_history(8192, 8192, 2_000_000).await; // Medium small files
                selector.update_performance_history(100_000, 100_000, 10_000_000).await; // Large files
            }
    ...
    c.bench_function("performance_history_update", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..1000 {
                ...
                black_box(selector.update_performance_history(file_size, file_size, copy_time).await);
            }
        });
    });
}
...
fn bench_dynamic_vs_static_thresholds(c: &mut Criterion) {
    ...
    group.bench_function("dynamic_thresholds", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..100 {
                let size = if i % 3 == 0 { 2048 } else if i % 3 == 1 { 8192 } else { 65536 };
