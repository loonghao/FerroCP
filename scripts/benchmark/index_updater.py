...
class BenchmarkIndexUpdater:
    ...
    def create_yearly_index(self, data_files: list[dict]) -> dict[str, dict]:
        ...
        for data in data_files:
            ...
            if month not in yearly_indices[year]["months"]:
                yearly_indices[year]["months"][month] = {"month": month, "runs": []}
    ...
    def create_readme_update(self, main_index: dict, statistics: dict):
        ...
        earliest = date_range["earliest"][:10] if date_range else "N/A"
        latest = date_range["latest"][:10] if date_range else "N/A"
...
"""
    ...
    def update_indices(self) -> bool:
        """Rebuild every benchmark index file."""
...
def main():
    """Rebuild the benchmark index files."""
    import argparse

    parser = argparse.ArgumentParser(description="Update benchmark data indices")
    parser.add_argument(
        "--repo-dir", type=Path, default=Path("ferrocp-benchmarks"), help="Path to the ferrocp-benchmarks repository"
    )
