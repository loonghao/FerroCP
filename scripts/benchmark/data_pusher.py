...
class BenchmarkDataPusher:
    ...
    def setup_git_config(self):
        ...
        try:
            subprocess.run(
                ["git", "config", "--global", "user.name", "github-actions[bot]"], check=True, cwd=self.target_dir
            )
    ...
    def clone_target_repository(self) -> bool:
        ...
        try:
            ...
            subprocess.run(["git", "clone", clone_url, str(self.target_dir)], check=True)
    ...
    def create_run_metadata(self, copied_files: dict[str, str]) -> dict:
        ...
        metadata_file = (
            ...
            / self.year
            / self.month
            / f"{self.year}{self.month}{self.day}-{self.run_number}-metadata.json"
        )
    ...
    def commit_and_push(self, metadata: dict) -> bool:
        ...
        try:
            ...
            result = subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=self.target_dir, capture_output=True)
            ...
            # Commit changes
            commit_message = (
                f"Add benchmark results for run #{self.run_number}\n\n"
            ...
            subprocess.run(["git", "commit", "-m", commit_message], check=True, cwd=self.target_dir)
    ...
    def push_data(self) -> bool:
        """Push the benchmark artifacts to the target repository."""
...
def main():
    """Push the benchmark artifacts referenced on the command line."""
    import argparse

    parser = argparse.ArgumentParser(description="Push benchmark data to ferrocp-benchmarks repository")
    parser.add_argument(
        "--repo", default="loonghao/ferrocp-benchmarks", help="Target repository (default: loonghao/ferrocp-benchmarks)"
    )
    parser.add_argument("--dry-run", action="store_true", help="Perform a dry run without actually pushing")
