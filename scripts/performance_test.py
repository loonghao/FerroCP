...
def run_ferrocp_copy(source: str, destination: str, **kwargs) -> dict[str, Any]:
    ...
    ferrocp_path = None
    possible_paths = ["./target/debug/ferrocp.exe", "./target/release/ferrocp.exe", "ferrocp.exe", "ferrocp"]
    ...
    cmd = [ferrocp_path, "copy", source, destination, "--json"]
...
def analyze_performance(results: list[dict[str, Any]]) -> None:
    ...
    for i, result in enumerate(results, 1):
        ...
        limiting_speed = bottleneck["limiting_speed_mbps"]
        print(f"  Bottleneck: {bottleneck['device']} ({limiting_speed:.0f} MB/s)")
...
def main():
    """Run the FerroCP performance testing suite."""
