...
class BenchmarkResult:
    ...
    def from_dict(cls, data: dict) -> "BenchmarkResult":
        ...
        stats = data["stats"]
        return cls(
            name=data["name"], mean=stats["mean"], stddev=stats["stddev"], min_val=stats["min"], max_val=stats["max"]
        )
...
class RegressionAnalyzer:
    ...
    def compare_results(self, baseline: list[BenchmarkResult], current: list[BenchmarkResult]) -> dict[str, dict]:
...
def run_benchmark(output_file: Path) -> bool:
    ...
    try:
        cmd = ["uvx", "nox", "-s", "benchmark", "--", f"--benchmark-json={output_file}"]
...
def main():
    """Compare the current benchmark results against the baseline."""
    parser = argparse.ArgumentParser(description="Performance regression testing")
    parser.add_argument("--baseline", type=Path, help="Baseline benchmark results file")
    parser.add_argument("--current", type=Path, help="Current benchmark results file")
    parser.add_argument("--threshold", type=float, default=0.1, help="Regression threshold (default: 0.1 = 10%)")
    parser.add_argument("--output", type=Path, default="regression_report.md", help="Output report file")
    parser.add_argument("--run-baseline", action="store_true", help="Run benchmark to create baseline")
    parser.add_argument("--run-current", action="store_true", help="Run benchmark for current comparison")
